...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateMoviesUI(movies) {
        ...
        movies.forEach(movie => {
            ...
            // ZMĚNA: Placeholder pro obrázky
            const imageHtml = movie.image ?
                `<img src="data:image/jpeg;base64,${movie.image}" alt="${movie.title}">` :
                movie.has_image ?
                `<img src="data:image/svg+xml;charset=utf-8,${encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="40" height="60"><rect width="100%" height="100%" fill="#666"/></svg>')}" alt="Loading..." data-movie-id="${movie.id_screening}">` :
            ...
            // Karta pro mobilní zobrazení - stejná změna
            const card = document.createElement('div');
            card.className = 'movie-card';
            const cardImageHtml = movie.image ?
                `<img src="data:image/jpeg;base64,${movie.image}" alt="${movie.title}" class="movie-image">` :
                movie.has_image ?
                `<img src="data:image/svg+xml;charset=utf-8,${encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="60" height="90"><rect width="100%" height="100%" fill="#666"/></svg>')}" alt="Loading..." class="movie-image" data-movie-id="${movie.id_screening}">` :
                '<div class="no-image">No image</div>';
            ...
            // NOVÉ: Lazy load obrázků i v admin panelu
            if (movie.has_image && !movie.image) {
                ...
                if (desktopImg) loadImageLazyAdmin(movie.id_screening, desktopImg);
                if (mobileImg) loadImageLazyAdmin(movie.id_screening, mobileImg);
            }
        });
    ...
    // Přidat na konec admin.js
    async function loadImageLazyAdmin(movieId, imgElement) {
        try {
